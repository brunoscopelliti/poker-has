 ...
 *
 * < output:
 * { strength: 1, rank: 'Q', kickers: ['J', '9', '4', '2'] }
 */
exports = module.exports = function hasFlush(cards) {
  ...
  const sortedCards = sortByRank(cards.map(card => card.rank));

  if (!hasHoles(sortedCards) || hasAceStraight(sortedCards, straightType.low)){
    return false;
  }

  let flushRank = sortedCards.shift();

  return { strength: rank.get('flush'), rank: flushRank, kickers: sortedCards };
